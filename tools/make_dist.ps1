...
# Usage: powershell -ExecutionPolicy Bypass -File tools\make_dist.ps1
...
Write-Host '==> [1/3] 暂存文件' -ForegroundColor Cyan
if (Test-Path $STAGE_ROOT) { Remove-Item -Recurse -Force $STAGE_ROOT }
New-Item -ItemType Directory -Force -Path $STAGE | Out-Null

$ExcludeDirs = @(
  'tmp', 'memory', '.git', 'node_modules', 'research',
...
robocopy $ROOT $STAGE /E /XD $ExcludeFull /XF *.pyc "*.zip" "*.tgz" /NFL /NDL /NJH /NJS /NP | Out-Null
